...
function searchclick() {
    ...
    $.ajax({
        ...
        cache: false,
        success: function (response) {
            let count_pincode = response.length;
            console.log(response);
            let html = "";
            html += '<li class="item" data-search-term="All"><a class="close-pincode-modal" href="'+home+'pincodeclear"> All</a></li>';
            if (count_pincode > 0) {
                for (let i = 0; i < count_pincode; i++) {
                    html += '<li class="item" data-search-term="' + response[i].id + '"><a class="close-pincode-modal" href="/pincode/' + response[i].id + '">' + response[i].pincode + "</a></li>";
                }
            }
